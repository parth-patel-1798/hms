...
const AppBar = ({ children, open, mobile, className, onClose }) => {
  ...
  const AppBarBackgroundVariant = cva(
    "fixed inset-0 bg-black transition-all duration-300 md:hidden",
    {
      variants: {
        open: {
  ...
  return (
    <>
      <aside
        className={mergeClasses(AppBarVariant({ open, mobile }), className)}
      >
        <nav className={mergeClasses(AppNavVariant({ open }))}>{children}</nav>
      </aside>
      <div
        className={mergeClasses(AppBarBackgroundVariant({ open }))}
        onClick={onClose}
      />
    </>
  );
